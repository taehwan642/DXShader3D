...
float3 MyReflect(float3 l, float3 N)
{
...
void Projection( float4 vPosObject: POSITION,
	             ...
	             out float4 vSeperateSpecularColorOut: COLOR1/*---*/,
                 uniform bool bSpecular )
{
	...
	vTexCoordOut.x = vTexCoordIn.x * cos( 0.1f * g_fTime ) - vTexCoordIn.y * sin( 0.1f * g_fTime );
	vTexCoordOut.y = vTexCoordIn.x * sin( 0.1f * g_fTime ) + vTexCoordIn.y * cos( 0.1f * g_fTime );
    ...
    if( bSpecular )
    {
		...
		float3 vCamera = normalize( g_vCameraPosition - vPosWorld.xyz );
		//float3 vReflection = reflect( -vLight, vNormalWorld );
		float3 vReflection = MyReflect(-vLight, vNormalWorld);

		float  fPhongValue = saturate( dot( vReflection, vCamera ) );
		
        //vColorOut.rgb += g_vMaterialSpecular * pow( fPhongValue, g_nMaterialShininess );
		vSeperateSpecularColorOut.rgb = g_vMaterialSpecular * pow( fPhongValue, g_nMaterialShininess );
		vSeperateSpecularColorOut.a = g_fMaterialAlpha;
	}
...
void Lighting( float2 vTexCoord: TEXCOORD0,
			   ...
			   float4 vSeperateSpecularColorIn: COLOR1/*---*/,
			   out float4 vColorOut: COLOR0,
...
{  
	...
	/*---*/
	if( bSpecular )
		vColorOut.rgb += vSeperateSpecularColorIn.rgb;
}
